...
from django.contrib.auth.decorators import login_required # decorators : 함수를 인자로 받음
...
# O -> new 함수 실행
# X -> login 페이지로 보냄
@login_required(login_url='/accounts/login/') # 메소드 
def new(request):
...
def delete(request, pk): # POST
    ...
    if request.user != article.user: # django-guardian
